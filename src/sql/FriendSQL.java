...
public class FriendSQL {
	...
	public static void loadUsersFriends() { // Carregar listas de amigos
		...
		for (UserClient uc : Logic.arUserClient) {
			...
			String friend = "SELECT friend_id, friend_friend_id, friend_status_id"
			+ " FROM Friend"
			...
			try {
				...
				while (rs.next()) {
					uc.getUserArFriend().add(new Friend(rs.getInt("friend_id"), sUser.searchUserClient(rs.getInt("friend_friend_id")), sStatus.searchStatus(rs.getInt("friend_status_id"))));
				}
				conn.close();
			...
			friend = "SELECT friend_id, friend_user_id"
			+ " FROM Friend"
			...
			try {
				...
				while (rs.next()) {
					uc.getUserArFriend().add(new Friend(rs.getInt("friend_id"), sUser.searchUserClient(rs.getInt("friend_user_id")), sStatus.searchStatus(rs.getInt("friend_status_id"))));
				}
				conn.close();
	...
	public static int addFriend(int friend_user_id, int friend_friend_id) { // Adicionar amigo
		try {
			...
			st.executeUpdate(
					"INSERT INTO Friend(friend_user_id, friend_friend_id, friend_status_id)"
					+ " VALUES ('" + friend_user_id + ", " + friend_friend_id + ","
	...
	public static void delFriendship(int friendship_id) { // Apagar relação de amizade
		String delFriendship = "DELETE FROM Friend"
				+ " WHERE friend_id = " + friendship_id + "";

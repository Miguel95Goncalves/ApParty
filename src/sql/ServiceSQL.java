...
public class ServiceSQL {
	...
	public static void loadService(){
		String services = "SELECT service_id, service_name, service_tiny_description, service_full_description, service_images, service_user_id, service_status_id, service_category_id"
				+ " FROM Services"
				+ " LEFT JOIN Status ON Status.status_id = Services.service_status_id"
		...
		try {
			...
			while (rs.next()) {
				Logic.arService.add(new Service(rs.getInt("service_id"), rs.getString("service_name"), rs.getString("service_tiny_description"), rs.getString("service_full_description"), rs.getString("service_images"), sStatus.searchStatus(rs.getInt("service_status_id")), sCategory.searchCategory(rs.getInt("service_category_id"))));
			}
			conn.close();
	...
	public static void loadServiceToPartys() {
		
		for(Party p : Logic.arParty) {
			String services = "SELECT ps_id, ps_service_id"
			+ " FROM Party_Service"
			+ " WHERE ps_party_id = " + p.getParty_id() + "";
	...
	public static int insertService(String service_name, String service_tiny_description,
			String service_full_description, String service_images,
			int service_category_id, int service_user_id) {
		...
		try {
			...
			st.executeUpdate(
					...
					+ " VALUES ('" + service_name + ", " + service_tiny_description + ", " + service_full_description + ", " + service_images + ", " + service_user_id + ","
	...
	public static void editService(int service_id, String service_name, String service_tiny_description,
			String service_full_description, String service_images,
			int service_category_id, int service_user_id) {
		...
		try {
			...
			st.executeUpdate("UPDATE Services" + " SET service_name = '" + service_name + "', service_tiny_description = '" + service_tiny_description
					+ "', service_full_description = '" + service_full_description + "'," + " service_images = '" + service_images
	...
	public static void delService(int service_id) {
		try {
			...
			+ " (SELECT status_id FROM Status LEFT JOIN Tables ON Tables.table_id = Status.status_table_id" 
			+ " WHERE table_name = 'Services' AND status_name = 'Disabled')"
			+ " WHERE service_id = " + service_id + "");
